...
def extract_keywords(data, search_keywords):
    # 创建一个空列表来存储匹配的结果
    matched_results = []
    # 遍历数据中的每个键值对
    for key, values in data['args'].items():
        # 检查每个值是否包含在搜索关键字列表中
        for value in values:
            if value in search_keywords:
                # 如果匹配到关键字，将键添加到匹配结果列表中
                matched_results.append(key)
                break  # 只需添加一次
...
# 测试函数
'''
data = {'args': {'感冒': ['disease'], '发烧': ['fever'], '头痛': ['headache']}, 'question_types': ['disease_do_food'],'鲫鱼砂仁羹':['food']}
